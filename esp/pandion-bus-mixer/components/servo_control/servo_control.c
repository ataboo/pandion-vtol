...
struct servo_ctrl_handle_impl {
    uint_t channel_duties[SERVO_CHANNEL_COUNT];
    uint_t gpio_nums[SERVO_CHANNEL_COUNT];
    uint_t ledc_channels[SERVO_CHANNEL_COUNT];
};
...
esp_err_t servo_ctrl_set_channel(servo_ctrl_handle_t handle, uint_t servo_channel, uint32_t duty) {
    handle->channel_duties[channel] = duty;

    ledc_set_duty(SERVO_LEDC_SPEED_MODE, handle->ledc_channels[channel], duty);
}
