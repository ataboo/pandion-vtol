...
static float max_roll_rate = 45.0;
static float max_pitch_rate = 45.0;
...
esp_err_t neutral_axis_stabilizer_update(transition_state_t transition_state, axis_duties_t* input_axes, gyro_values_t* gyro_values) {
    ...
    if (transition_state == TRANS_HORIZONTAL) {
        ...
        input_axes->roll = get_pid_output(roll_pid_v, max_roll_rate * input_axes->roll, gyro_values->norm_gyro_x);
        input_axes->pitch = get_pid_output(pitch_pid_v, max_pitch_rate * input_axes->pitch, gyro_values->norm_gyro_y);
